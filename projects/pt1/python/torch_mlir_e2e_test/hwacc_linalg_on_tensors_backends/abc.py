...
# A type shared between the result of `HwaccLinalgOnTensorsBackend.compile` and the
# input to `HwaccLinalgOnTensorsBackend.load`. Each backend will likely have a
...
class HwaccLinalgOnTensorsBackend(abc.ABC):
    ...
    def compile(self, module: Module) -> CompiledArtifact:
        ...
        (see the VerifyHwaccLinalgOnTensorsBackendContract pass).
